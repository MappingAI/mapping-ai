  ...
  try {
    ...
    try {
      ...
      if (contributorKey) {
        ...
        contributorKeyId = keyResult.rows[0].id as number
        const dailyLimit = (keyResult.rows[0].daily_limit as number) || 20
        ...
        if (parseInt(rateResult.rows[0].count as string, 10) >= dailyLimit) {
          client.release()
      ...
      submissionId = result.rows[0].id as number
    } finally {
      client.release()
    }
