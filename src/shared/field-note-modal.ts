...
import { getVoterId } from './field-feedback-utils'
...
interface OpenModalOptions {
  ...
  entityName?: string
  fieldLabel?: string
  onNoteAdded?: (text: string, html: string | null) => void
}
...
export function openFieldNoteModal({ entityId, field, entityName, fieldLabel, onNoteAdded }: OpenModalOptions) {
  ...
  let tiptapEditor: EditorLike | null = null
  ...
  function getEditorContent() {
    ...
    return { text: ta ? ta.value.trim() : '', html: null as string | null, mentions: [] as unknown[] }
  }
  ...
  function doSubmit() {
    ...
    fetch('/api/field-notes', {
      ...
      .then((r) => {
        if (r.ok) {
          if (onNoteAdded) onNoteAdded(text, html ? DOMPurify.sanitize(html) : null)
          close()
        } else {
  ...
  import('../map/field-note-editor.js')
    .then(({ createFieldNoteEditor }) => {
      tiptapEditor = createFieldNoteEditor(editorContainer) as EditorLike
      setTimeout(() => {
