...
// bridges column names (`belief_regulatory_stance` → `regulatory_stance`).
//
...
export interface DbEntityRow {
  ...
  created_at: string
  updated_at: string
  // tsvector is opaque; never send to the client — it's excluded by SENSITIVE.
...
export interface DbSubmissionRow {
  ...
  // Submitter — PII. Never export to public map.
 ...
 * Never shipped to the client or to public fixtures; holds PII (name, email).
 */
...
/**
 * Fields that must never leave the backend. Check against this set in every
 * path that serializes a row to the client (public map, search responses,
 * fixture generator). Matches api/export-map.js `SENSITIVE` constant.
 */
export const SENSITIVE_ENTITY_FIELDS = new Set<keyof DbEntityRow>(['search_vector'])

export const SENSITIVE_SUBMISSION_FIELDS = new Set<keyof DbSubmissionRow>([
  'submitter_email',
