  ...
  return (
    <div className="mt-2 border border-amber-300 rounded bg-amber-50 overflow-hidden">
      ...
      <div className="flex items-center gap-1.5 px-3 py-2 bg-amber-100 border-b border-amber-200">
        ...
        {isLoadingPending && (
          <span className="text-[10px] font-mono text-amber-600 ml-auto">checking...</span>
        )}
      </div>
          ...
          <div key={match.id} className="flex items-start justify-between gap-3 px-3 py-2">
            ...
            <div className="min-w-0 flex-1">
              <div className="flex items-center gap-1.5">
                <span className="text-[13px] font-mono font-medium text-[#1a1a1a] truncate">
                  {match.name}
                </span>
                {match.isPending && (
              ...
              <div className="flex flex-wrap items-center gap-x-2 gap-y-0.5 mt-0.5">
                {match.category && (
                  <span className="text-[11px] font-mono text-[#666]">{match.category}</span>
                )}
                {match.title && (
                  <span className="text-[11px] font-mono text-[#888] truncate max-w-[180px]">
                    {match.title}
                  </span>
                )}
                {match.primary_org && (
                ...
                {match.location && (
                  <span className="text-[11px] font-mono text-[#999] truncate max-w-[140px]">
                    {match.location}
                  </span>
                )}
              </div>
