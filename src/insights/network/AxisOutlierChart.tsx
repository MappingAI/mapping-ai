...
export function AxisOutlierChart({ entities, mode }: AxisOutlierChartProps) {
  ...
  useEffect(() => {
    ...
    labelsX.forEach((label, i) => {
      const x = xScale(i + 1)
      xAxisG.append('line').attr('x1', x).attr('x2', x).attr('y1', 0).attr('y2', 6).attr('stroke', '#ccc')
    ...
    if (mode === '2d' && configY) {
      ...
      labelsY.forEach((label, i) => {
        const y = yScale(i + 1)
        yAxisG.append('line').attr('x1', -6).attr('x2', 0).attr('y1', y).attr('y2', y).attr('stroke', '#ccc')
    ...
    const nodeData = validEntities.map((e, i) => {
      ...
      return {
        ...
        y:
          mode === '2d' ? yScale(scoreY) + (Math.random() - 0.5) * 20 : plotH / 2 + (Math.random() - 0.5) * plotH * 0.8,
        radius: nodeRadius,
        index: i,
      }
    })
    ...
    if (labelNodes.length > 0) {
      ...
      const labelSim = d3
        ...
        .force(
          'collide',
          d3.forceCollide((d: (typeof labelNodes)[0]) => Math.max(d.labelWidth, d.labelHeight) / 2 + 8).strength(1),
        )
        .force('avoidNodes', () => {
