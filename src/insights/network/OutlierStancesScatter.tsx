...
// D3 selection type (d3 loaded from CDN, no compile-time types)
type D3Sel = {
  select: (s: string) => D3Sel
  selectAll: (s: string) => D3Sel
  attr: (k: string, v: string | number) => D3Sel
}
...
export function OutlierStancesScatter({ entities, onSelectEntity }: OutlierStancesScatterProps) {
  ...
  useEffect(() => {
    ...
    svg
      ...
      .call((g: D3Sel) => g.select('.domain').attr('stroke', '#ccc'))
      .call((g: D3Sel) => g.selectAll('.tick line').attr('stroke', '#ccc'))
      .call((g: D3Sel) =>
        g
          .selectAll('.tick text')
          .attr('font-family', "'DM Mono', monospace")
