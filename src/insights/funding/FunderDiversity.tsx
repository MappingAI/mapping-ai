    ...
    >
      <div className="bg-white rounded-lg shadow-xl max-w-[420px] w-full overflow-hidden">
        <div className="flex justify-between items-start p-4 pb-2 border-b border-[#eee]">
          <div>
            <div className="font-mono text-[10px] tracking-[0.08em] uppercase text-[#888] mb-1">Funder Diversity</div>
        ...
        <div className="p-4">
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="font-mono text-[10px] text-[#888]">{detail.org.uniqueCategories} funder categories</span>
          </div>
...
export function FunderDiversity({ edges, showTooltip, hideTooltip }: Props) {
  ...
  useEffect(() => {
    ...
    const orgData: OrgData[] = Array.from(byRecipient.entries())
      .map(([name, items]) => {
        const funders = [
          ...new Map(items.map((e) => [e.funder, { name: e.funder, category: e.funder_category }])).values(),
        ]
        const categoryCounts = Array.from(
          d3.rollup(
            funders,
            (v: Array<{ name: string; category: string }>) => v.length,
            (f: { name: string; category: string }) => f.category,
    ...
    const singleCategoryOrgs = orgData
      .filter((o) => o.isSingleCategory)
    ...
    const usedCategories = [...new Set(orgData.flatMap((o) => o.funders.map((f) => f.category)))]
