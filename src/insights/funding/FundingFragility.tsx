...
function DetailCardModal({ detail, onClose }: { detail: DetailCard; onClose: () => void }) {
  const riskLevel =
    detail.recipient.topFunderShare > 0.6
      ? 'High'
      : detail.recipient.topFunderShare > 0.4
        ? 'Moderate'
        : 'Low'
  const riskColor =
    detail.recipient.topFunderShare > 0.6
      ...
      <div className="bg-white rounded-lg shadow-xl max-w-[420px] w-full overflow-hidden">
        ...
        <div className="p-4 space-y-3">
          ...
          {detail.recipient.funders.map((f, i) => (
            <div key={i} className="flex items-center gap-2">
              ...
              <div className="font-mono text-[10px] text-[#666] flex-shrink-0">
        ...
        <div className="p-3 pt-2 border-t border-[#eee] bg-[#faf9f7]">
          <div className="font-mono text-[9px] text-[#888]">
            {detail.recipient.funders.length} unique funders ·{' '}
            {detail.recipient.uniqueFunderCategories} funder categories ·{' '}
            {detail.recipient.totalGrants} total grants
          </div>
        </div>
      </div>
    </div>
...
export function FundingFragility({ edges, showTooltip, hideTooltip }: Props) {
  ...
  useEffect(() => {
    ...
    const byRecipient = d3.group(policyEdges, (e: FundingEdge) => e.recipient) as Map<
      string,
      FundingEdge[]
    >
    ...
    ;[
      ...
      const x = 90 + i * 95
      svg.append('circle').attr('cx', x).attr('cy', legendY - 3).attr('r', 4).attr('fill', item.color)
      svg
        .append('text')
        .attr('x', x + 8)
        .attr('y', legendY)
  ...
  return (
    <>
      <div ref={ref} />
      {selectedRecipient && (
        <DetailCardModal detail={selectedRecipient} onClose={() => setSelectedRecipient(null)} />
      )}
    </>
  )
}
