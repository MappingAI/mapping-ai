      ...
      <div className="bg-white rounded-lg shadow-xl max-w-[600px] w-full max-h-[70vh] overflow-hidden flex flex-col">
            ...
            {sortedEdges.map((edge, i) => (
              <div key={i} className="bg-[#f8f7f5] rounded-md p-3">
                <div className="flex justify-between items-start gap-3">
                  <div className="font-mono text-[12px] font-medium text-[#333] flex-1">{edge.recipient}</div>
        ...
        <div className="p-4 pt-3 border-t border-[#eee] bg-[#faf9f7]">
          <div className="font-mono text-[10px] text-[#888]">
            Total: {formatUSD(flow.edges.reduce((sum, e) => sum + (e.amount_usd || 0), 0))} across {flow.edges.length}{' '}
            investments
          </div>
        </div>
      </div>
    ...
    linkGroup
      ...
      .on(
        ...
        function (
          this: SVGPathElement,
          evt: MouseEvent,
          d: { source: { name: string }; target: { name: string }; value: number },
        ) {
          ...
          tooltip
            .style('opacity', 1)
            .html(
              `<strong>${d.source.name}</strong> → ${d.target.name}<br>${d.value} investments<br><em style="font-size:9px;color:#888">Click for details</em>`,
            )
      ...
      .on('click', function (_evt: MouseEvent, d: { source: { name: string }; target: { name: string } }) {
        ...
        const flowEdges = edges.filter((e) => e.funder === d.source.name && e.recipient_category === d.target.name)
        setSelectedFlow({
