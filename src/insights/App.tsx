...
function ChartAxisDistributions({ entities }: { entities: Entity[] }) {
  ...
  useEffect(() => {
    ...
    const rowH = 22, gap = 3, sectionGap = 30
    const padL = 110, padR = 50, titleH = 18
    ...
    let totalH = 0
    axes.forEach(ax => { totalH += titleH + ax.labels.length * (rowH + gap) + sectionGap })
    totalH += 10
    ...
    const maxCount = d3.max(axes.flatMap((ax: { labels: string[]; key: string }) =>
      ax.labels.map((_: string, i: number) => entities.filter(e => Math.round(e[ax.key]) === i + 1).length)
    ))
    ...
    axes.forEach(ax => {
      svg.append('text')
        .attr('x', 0).attr('y', yOffset + 12)
        .attr('font-family', "'DM Mono', monospace").attr('font-size', 10)
        .attr('font-weight', 500).attr('fill', '#555')
        .text(ax.name)
      yOffset += titleH

      ax.labels.forEach((label: string, i: number) => {
        const count = entities.filter(e => Math.round(e[ax.key]) === i + 1).length
        const y = yOffset

        svg.append('text')
          .attr('x', padL - 8).attr('y', y + rowH / 2 + 1)
          .attr('text-anchor', 'end').attr('dominant-baseline', 'middle')
          .attr('font-family', "'DM Mono', monospace").attr('font-size', 10).attr('fill', '#666')
          .text(label)

        svg.append('rect')
          .attr('x', padL).attr('y', y)
          .attr('width', xScale(count)).attr('height', rowH)
          .attr('rx', 3).attr('fill', '#2563eb').attr('opacity', 0.6)

        svg.append('text')
          .attr('x', padL + xScale(count) + 6).attr('y', y + rowH / 2 + 1)
...
function ChartFundingStance({ orgs }: { orgs: Entity[] }) {
  ...
  useEffect(() => {
    ...
    const barH = 28, gap = 6, padL = 180, padR = 80
    ...
    stats.forEach((s: { funding: string; count: number; mean: number; std: number }, i: number) => {
      ...
      svg.append('line')
        .attr('x1', xScale(errLeft)).attr('x2', xScale(errRight))
        .attr('y1', y + barH / 2).attr('y2', y + barH / 2)
        .attr('stroke', '#2563eb').attr('stroke-width', 2).attr('opacity', 0.3)

      // Mean dot
      const circle = svg.append('circle')
        .attr('cx', xScale(s.mean)).attr('cy', y + barH / 2)
        .attr('r', 8)
        .attr('fill', '#2563eb')
      ...
      // Label
      svg.append('text')
        .attr('x', xScale(s.mean) + 12).attr('y', y + barH / 2 - 10)
        .attr('dominant-baseline', 'middle')
        .attr('font-family', "'DM Mono', monospace").attr('font-size', 9).attr('fill', '#666')
...
function ChartThreatFrequency({ entities }: { entities: Entity[] }) {
  ...
  useEffect(() => {
    ...
    const THREATS = [
      'Loss of control', 'Existential risk', 'Power concentration', 'Labor displacement',
      'Economic inequality', 'Democratic erosion', 'Misinformation', 'National security',
      'Bias/discrimination', 'Privacy', 'Cybersecurity', 'Weapons proliferation', 'Environmental', 'Copyright/IP',
    ]
    ...
    const sorted = THREATS.filter(t => counts[t]).map(t => ({ label: t, count: counts[t] || 0 })).sort((a, b) => b.count - a.count)
    if (sorted.length === 0) return

    const W = container.clientWidth || 660
    const barH = 22, gap = 4, padL = 160, padR = 50, padTop = 45
    ...
    // Legend
    svg.append('rect').attr('x', padL).attr('y', 8).attr('width', 12).attr('height', 12).attr('rx', 2).attr('fill', '#1F78B4')
    svg.append('text').attr('x', padL + 16).attr('y', 17).attr('font-family', "'DM Mono', monospace").attr('font-size', 9).attr('fill', '#555').text('Technical/x-risk')
    svg.append('rect').attr('x', padL + 110).attr('y', 8).attr('width', 12).attr('height', 12).attr('rx', 2).attr('fill', '#FF7F00')
    svg.append('text').attr('x', padL + 126).attr('y', 17).attr('font-family', "'DM Mono', monospace").attr('font-size', 9).attr('fill', '#555').text('Societal/near-term')
    ...
    sorted.forEach((d, i) => {
      ...
      svg.append('rect')
        ...
        .attr('opacity', 0.65)
        .on('mousemove', (e: MouseEvent) => showTooltip(e,
...
function ChartThreatCooccurrence({ entities }: { entities: Entity[] }) {
  ...
  useEffect(() => {
    ...
    const THREATS = [
      'Loss of control', 'Existential risk', 'Power concentration', 'Labor displacement',
      'Economic inequality', 'Democratic erosion', 'Misinformation', 'Bias/discrimination',
    ]
    ...
    THREATS.forEach((t, i) => {
      svg.append('text')
        .attr('x', labelPadLeft - 8).attr('y', labelPadTop + i * cellSize + cellSize / 2)
        .attr('text-anchor', 'end').attr('dominant-baseline', 'middle')
        .attr('font-family', "'DM Mono', monospace").attr('font-size', 9).attr('fill', '#555')
        .text(t)
    })
    ...
    matrix.forEach((row: number[], i: number) => {
      row.forEach((val: number, j: number) => {
        ...
        svg.append('rect')
          .attr('x', labelPadLeft + j * cellSize).attr('y', labelPadTop + i * cellSize)
          .attr('width', cellSize - 2).attr('height', cellSize - 2)
          .attr('fill', colorScale(val))
          .attr('rx', 3)
          .on('mousemove', (e: MouseEvent) => showTooltip(e,
            `${THREATS[i]} + ${THREATS[j]}<br>Jaccard: <strong>${val.toFixed(2)}</strong>`))
          .on('mouseleave', () => hideTooltip())
    ...
    matrix.forEach((row: number[], i: number) => {
      row.forEach((val: number, j: number) => {
        svg.append('rect')
          ...
          .attr('rx', 3)
          .on('mousemove', (e: MouseEvent) => showTooltip(e,
            `${categories[i]} ↔ ${categories[j]}<br><strong>${val} edges</strong>`))
          .on('mouseleave', () => hideTooltip())
      })
    })
  }, [edges, entities])
...
export function App() {
  ...
  const dataDate = useMemo(() => data?._meta?.generated_at
  ...
  const { maxStance, minStance } = useMemo(() => {
    const stanceCounts = STANCE_LABELS.map((_, i) => allEntities.filter(e => Math.round(e.stance_score || 0) === i + 1).length)
  ...
  return (
    <>
      ...
      {/* Article */}
      <article className="max-w-[760px] mx-auto font-serif text-[#1a1a1a] text-[17px] leading-[1.75]" style={{ padding: 'calc(3rem + 48px) 1.5rem 4rem' }}>
        ...
        <p className="font-serif text-[18px] text-[#555] leading-[1.6] mb-8">
          We mapped <span className="font-mono">{allWithResources.length || '—'}</span> entities in U.S. AI policy. Rather than claiming what percentage believes what, we focus on <em>structure</em>: how the ecosystem is organized, who connects to whom, and where the gaps lie.
        </p>
        ...
        <div id="overview" className="flex gap-4 my-6 flex-wrap max-[600px]:flex-col">
          ...
          <div className="flex-1 min-w-[120px] bg-[#f8f7f5] rounded-md p-4 text-center">
            <div className="font-mono text-[26px] font-medium text-[#1a1a1a] leading-[1.2]">{resources.length || '—'}</div>
        ...
        <Para>
          This page presents structural analyses of the Mapping AI database. Our guiding principle: avoid selection bias by focusing on in-group comparisons rather than extrapolating to the broader population. We're interested in relational patterns—who clusters with whom, what beliefs travel together, which communities are bridged or siloed.
        </Para>
        ...
        <Para>
          We track three belief dimensions for people and organizations: <strong>regulatory stance</strong> (accelerate → precautionary), <strong>AGI timeline</strong> (already here → 25+ years), and <strong>AI risk level</strong> (overstated → existential). These form a three-dimensional "belief space" where entities can be positioned.
        </Para>

        <ChartContainer title="Belief axis distributions (people + orgs with data)" source="Horizontal bars show count at each level. Entities without data excluded.">
          <ChartAxisDistributions entities={allEntities} />
        </ChartContainer>

        <Para>
          The distributions reveal which positions are crowded and which are sparse. A heavily populated region suggests mainstream consensus; empty regions may indicate underrepresented perspectives or simply positions few people hold.
        </Para>

        <Finding>
          The belief space is unevenly populated. "{maxStance}" is the most common regulatory stance, while "{minStance}" is least represented. This concentration suggests either consensus or sampling bias—distinguishing between them requires external validation.
        </Finding>

        <h3 className="font-serif text-[18px] font-normal mt-8 mb-3">Funding model → ideology</h3>

        <Para>
          Does knowing how an organization is funded predict its regulatory stance? This isn't necessarily causal—funders may seek out aligned orgs, not shape them—but the correlation is structurally important.
        </Para>

        <ChartContainer title="Mean regulatory stance by funding model" source="Scale: 1 = Accelerate, 6 = Precautionary. Only funding models with 5+ orgs shown.">
          <ChartFundingStance orgs={orgs} />
        </ChartContainer>

        <Finding>
          Funding source is predictive of regulatory stance. Philanthropically-funded organizations skew more restrictive, while venture-backed organizations favor lighter regulation. This structural pattern holds regardless of whether funders actively influence grantees or simply select for aligned orgs.
        </Finding>
        ...
        <Para>
          Entities identify which AI-related concerns they prioritize: loss of control, labor displacement, existential risk, democratic erosion, and more. These "threat models" reveal what problems different communities are trying to solve.
        </Para>

        <ChartContainer title="Most frequently cited concerns" source="Count of entities selecting each concern. Entities can select multiple.">
        ...
        <Para>
          Which concerns travel together? If someone cites "loss of control," are they likely to also cite "existential risk"? The co-occurrence matrix reveals concern clusters—sets of worries that form coherent worldviews.
        </Para>

        <ChartContainer title="Threat model co-occurrence (Jaccard similarity)" source="Higher values = concerns frequently appear together. Upper triangle shown to avoid redundancy.">
          <ChartThreatCooccurrence entities={allEntities} />
        </ChartContainer>

        <Finding>
          Technical/x-risk concerns (loss of control, existential risk) cluster together, as do societal/near-term concerns (labor displacement, economic inequality). These two worldviews represent distinct communities with different policy priorities—and they rarely cite each other's concerns.
        </Finding>
        ...
        <Para>
          The database tracks <span className="font-mono">{edges.length || '—'}</span> relationships between entities: affiliations, collaborations, funding, citations, and more. How connected is each stakeholder category to others? A category with high external connectivity serves as a hub or bridge; one with low connectivity may be siloed.
        </Para>

        <ChartContainer title="Cross-category edge counts (who connects to whom)" source="Cell color = log₁₀(edges) between categories. Logarithmic scale shows relative connectivity.">
          <ChartCategoryMatrix edges={edges} entities={allWithResources} />
        </ChartContainer>

        <Finding>
          Government and Think Tank categories have the most cross-category connections, serving as the primary hubs of the governance network. Categories like Media/Journalism are more internally focused with fewer bridges to other communities.
        </Finding>
        ...
        <div className="flex justify-center items-center gap-2 mt-12 pt-6 border-t border-[#bbb]/50">
          ...
          <a href="/map" className="font-mono text-[10px] text-[#2563eb] no-underline hover:underline tracking-[0.04em]">Explore the map →</a>
        </div>
      </article>
    </>
  )
