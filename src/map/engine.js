/* eslint-disable no-undef, no-dupe-keys, no-useless-escape, @typescript-eslint/no-unused-vars, @typescript-eslint/ban-ts-comment */
// @ts-nocheck
//
// Map engine — extracted from inline <script> in map.html.
// Wrapped in initMapEngine() so React can call it after mounting the DOM.
// d3 is expected as a global (loaded from CDN before this runs).
//
// DO NOT refactor the internals. This is a direct lift of the inline code.

export function initMapEngine() {
  // Search state (hoisted: referenced by getVisibleNodes before search section initializes)
  ...
  var _savedMode = localStorage.getItem('mapMode') || 'network'
  let viewMode = _savedMode === 'network' || _savedMode === 'plot' ? _savedMode : 'network'
  ...
  function applyViewState() {
    ...
    var searchModeCtls = document.getElementById('search-mode-controls')
    if (searchModeCtls) searchModeCtls.style.display = isSearch ? '' : 'none'
  ...
  if (viewMode === 'search') {
    var searchModeCtrls2 = document.getElementById('search-mode-controls')
    if (searchModeCtrls2) searchModeCtrls2.style.display = ''
    document.getElementById('stance-legend').style.display = 'none'
    document.getElementById('network-sub-tabs').style.display = 'none'
    const smallSearchGroup = document.querySelector('.controls .control-group:first-child')
  ...
  // Top-level mode buttons (legacy, kept for compatibility)
  ...
  // React-driven view mode changes
  function _onReactViewModeChange(e) {
    var mode = e.detail && e.detail.viewMode
    if (mode === 'network' || mode === 'plot') {
      viewMode = mode
      applyViewState()
    }
  }
  window.addEventListener('map:viewModeChange', _onReactViewModeChange)
  ...
  // (search state variables hoisted to top — see initMapEngine preamble)
  ...
  // Search button click (guard: element may not exist if search tab removed)
  if (searchRunBtn)
  ...
  if (searchModeInput) {
    searchModeInput.addEventListener('keydown', (e) => {
      ...
      if (e.key === 'Escape') {
        ...
        if (searchModeStatus) searchModeStatus.textContent = ''
      }
    })
  }
  ...
  if (searchClearBtn) {
    searchClearBtn.addEventListener('click', () => {
      ...
      searchModeMatches = []
      searchMatchReasons = {}
      showOneHopConnections = true
      if (showConnectionsToggle) showConnectionsToggle.checked = true
    })
  }
  ...
  // ── Sidebar init (contribute panel, onboarding, mobile banner) ──
  ...
  // ── Cleanup: return a destroy() handle for React unmount ──
  return {
    destroy() {
      // Stop D3 simulation
      if (simulation) simulation.stop()
      ...
      window.removeEventListener('map:viewModeChange', _onReactViewModeChange)
