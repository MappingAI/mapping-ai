    ...
    if (method === 'POST') {
      ...
      if (action === 'approve') {
        ...
        const check = await client.query(
          `SELECT id, entity_id FROM submission WHERE id = $1 AND status = 'pending'`,
          [submission_id]
        );
        if (check.rows.length === 0) {
          return { statusCode: 404, headers: CORS_HEADERS, body: JSON.stringify({ error: 'Submission not found or already reviewed' }) };
        }
        if (check.rows[0].entity_id !== null) {
          return { statusCode: 400, headers: CORS_HEADERS, body: JSON.stringify({ error: 'Use action=merge for edit submissions' }) };
        }
        ...
        await client.query(
          `UPDATE submission SET ${setClauses.join(', ')} WHERE id = $1`,
          values
        );
      ...
      if (action === 'merge') {
        ...
        const sub = await client.query(
          `SELECT entity_id FROM submission WHERE id = $1 AND status = 'pending'`,
          [submission_id]
        );
        ...
        if (!entityId) {
          return { statusCode: 400, headers: CORS_HEADERS, body: JSON.stringify({ error: 'Use action=approve for new entity submissions' }) };
        }
      ...
      if (action === 'update_entity') {
        ...
        if (!entity_id || !data) {
          return { statusCode: 400, headers: CORS_HEADERS, body: JSON.stringify({ error: 'Missing entity_id or data' }) };
        }
        const updates = [];
        const values = [];
        let idx = 1;
