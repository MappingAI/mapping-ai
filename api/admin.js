...
// Field presence semantics: update_entity uses Object.prototype.hasOwnProperty.call
// on each ENTITY_FIELDS key — absent = skip the column, `null`/`[]` = clear it.
// This avoids the thumbnail_url blind-overwrite bug class (see
...
const ENTITY_FIELDS = [
  ...
  // Resources Rethink Phase 1 — multi-tag arrays + advocated beliefs + source.
  'topic_tags',
  'format_tags',
  'advocated_stance',
  'advocated_timeline',
...
// Columns that are Postgres arrays (TEXT[]). Writes must cast explicitly in
// parameterized SQL to avoid pg driver binding edge cases on empty arrays.
const ARRAY_FIELDS = new Set(['topic_tags', 'format_tags'])

// Build a parameterized SET clause against ENTITY_FIELDS. Iterates the
// source object so absent keys are skipped (field-absent = leave alone,
// explicit null/[] = clear). Array columns get an ::text[] cast.
function buildEntitySet(data, startIdx = 1) {
  const updates = []
  const values = []
  let idx = startIdx
...
export const handler = async (event) => {
  ...
  try {
    ...
    if (method === 'GET' && params.action === 'pending') {
      ...
      return { statusCode: 200, headers: CORS_HEADERS, body: JSON.stringify({ submissions: result.rows }) }
    }
    ...
    if (method === 'POST') {
      ...
      if (action === 'approve') {
        const { submission_id, data: overrides = {} } = body
        if (!submission_id) {
        ...
        const setClauses = overrideFields.map((k, i) =>
          ARRAY_FIELDS.has(k) ? `${k} = $${i + 2}::text[]` : `${k} = $${i + 2}`,
        )
      ...
      if (action === 'merge') {
        const { submission_id, merged_data = {}, resolution_notes } = body
        if (!submission_id) {
          return { statusCode: 400, headers: CORS_HEADERS, body: JSON.stringify({ error: 'Missing submission_id' }) }
        }
        ...
        if (entityRow.rows.length === 0) {
          return { statusCode: 404, headers: CORS_HEADERS, body: JSON.stringify({ error: 'Entity not found' }) }
        }
        const current = entityRow.rows[0]

        if (Object.keys(merged_data).length > 0) {
          // Admin explicitly selected these fields for merge — allow clearing.
          const { updates, values, nextIdx } = buildEntitySet(merged_data, 1)
          if (updates.length > 0) {
            values.push(entityId)
            await client.query(
              `UPDATE entity SET ${updates.join(', ')}, updated_at = NOW() WHERE id = $${nextIdx}`,
              values,
            )
          }
        }
      ...
      // Field-presence semantics: `data` keys NOT in the body are skipped; keys
      // present with `null` / `[]` / any value are written through. Avoids the
      // thumbnail_url blind-overwrite bug class (see 2026-04-19 thumbnail post-mortem).
      if (action === 'update_entity') {
        ...
        const { updates, values, nextIdx } = buildEntitySet(data, 1)
        if (updates.length === 0) {
          return { statusCode: 400, headers: CORS_HEADERS, body: JSON.stringify({ error: 'No valid fields' }) }
        }
        values.push(entity_id)
        await client.query(`UPDATE entity SET ${updates.join(', ')}, updated_at = NOW() WHERE id = $${nextIdx}`, values)
        await refreshMapData(client)
      ...
      if (action === 'delete') {
        ...
        if (!entity_id) {
          return { statusCode: 400, headers: CORS_HEADERS, body: JSON.stringify({ error: 'Missing entity_id' }) }
        }
