...
function toFrontendShape(row) {
  ...
  const out = {
    ...
    status: row.status,
    // Provenance (enrichment skill, 2026-04-20). notes_sources is an array of
    // {url, snippet, retrieved_at, retriever, field_name?}. Fields default to
    // null/empty on legacy rows until a new enrichment writes them.
    notes_sources: row.notes_sources || null,
    notes_confidence: row.notes_confidence ?? null,
    enrichment_version: row.enrichment_version || null,
  }
