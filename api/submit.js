...
export const handler = async (event) => {
  ...
  try {
    ...
    for (const [key, value] of Object.entries(data)) {
      ...
      const limit = xlongFields.has(key) ? XLONG_LIMIT : longFields.has(key) ? LONG_LIMIT : SHORT_LIMIT
    ...
    try {
      ...
      // notesSources: array of { url, snippet, retrieved_at, retriever, field_name? }
      // notesConfidence: 1-5 integer. enrichmentVersion: free-form prompt/pipeline tag.
      let notesSources = null
      if (data.notesSources != null) {
        notesSources = typeof data.notesSources === 'string' ? JSON.parse(data.notesSources) : data.notesSources
      }
      const notesConfidence = data.notesConfidence != null ? parseInt(data.notesConfidence, 10) : null
      if (notesConfidence != null && (notesConfidence < 1 || notesConfidence > 5)) {
        // client.release() runs via the `finally` block below, no explicit release here
        return {
          statusCode: 400,
          headers: CORS_HEADERS,
          body: JSON.stringify({ error: 'notesConfidence must be between 1 and 5' }),
        }
      }
      const enrichmentVersion = data.enrichmentVersion || null
      ...
      const result = await client.query(
        ...
        [
          ...
          notesSources ? JSON.stringify(notesSources) : null,
          notesConfidence,
          enrichmentVersion,
          ts,
        ],
