...
async function verify() {
  ...
  for (const backupEdge of edgeSamples) {
    ...
    const srcResult = await pool.query('SELECT name FROM entity WHERE id = $1', [
      backupEdge.source_id,
    ])
    const tgtResult = await pool.query('SELECT name FROM entity WHERE id = $1', [
      backupEdge.target_id,
    ])
    ...
    if (dbEdge) {
      ...
      console.log(
        '[Edge] ' + srcName + ' → ' + tgtName + ' (' + backupEdge.edge_type + ') - MISSING ✗',
      )
    }
  }
  ...
  for (const name of correctNames) {
    const result = await pool.query('SELECT id, name, importance FROM entity WHERE name = $1', [
      name,
    ])
    if (result.rows.length > 0) {
      console.log(
        '[' + result.rows[0].id + '] ' + name + ' (importance=' + result.rows[0].importance + ') ✓',
      )
    } else {
  ...
  const submissions = await pool.query(
    'SELECT id, entity_type, name, resource_title, status FROM submission ORDER BY id',
  )
