...
const stagingEntities = JSON.parse(fs.readFileSync('backups/staging-20260415-with-importance/entities.json'));
const prodSubmissions = JSON.parse(fs.readFileSync('backups/production-20260415/submissions.json'));
const stagingSubmissions = JSON.parse(fs.readFileSync('backups/staging-20260415-with-importance/submissions.json'));
const prodEdges = JSON.parse(fs.readFileSync('backups/production-20260415/edges.json'));
const stagingEdges = JSON.parse(fs.readFileSync('backups/staging-20260415-with-importance/edges.json'));
...
if (subInProdOnly.length > 0) {
  for (const s of subInProdOnly) {
    console.log('  [' + s.id + '] ' + (s.name || s.resource_title || 'unnamed') + ' (' + s.entity_type + ') - status: ' + s.status + ' - created: ' + s.created_at);
  }
}
...
if (subInStagingOnly.length > 0 && subInStagingOnly.length <= 10) {
  for (const s of subInStagingOnly.slice(0, 10)) {
    console.log('  [' + s.id + '] ' + (s.name || s.resource_title || 'unnamed') + ' (' + s.entity_type + ') - status: ' + s.status);
  }
}
...
for (const s of pendingProd) {
  console.log('  [' + s.id + '] ' + (s.name || s.resource_title || 'unnamed') + ' (' + s.entity_type + ') - ' + s.created_at);
}
...
console.log('\n=== EDGE COMPARISON ===');
const prodEdgeKeys = new Set(prodEdges.map(e => e.source_id + '-' + e.target_id + '-' + e.edge_type));
const stagingEdgeKeys = new Set(stagingEdges.map(e => e.source_id + '-' + e.target_id + '-' + e.edge_type));

const edgesInProdOnly = prodEdges.filter(e => !stagingEdgeKeys.has(e.source_id + '-' + e.target_id + '-' + e.edge_type));
const edgesInStagingOnly = stagingEdges.filter(e => !prodEdgeKeys.has(e.source_id + '-' + e.target_id + '-' + e.edge_type));
...
if (edgesInProdOnly.length > 0) {
  ...
  for (const e of edgesInProdOnly.slice(0, 5)) {
    ...
    console.log('  ' + (src ? src.name : e.source_id) + ' → ' + (tgt ? tgt.name : e.target_id) + ' (' + e.edge_type + ')');
  }
}
...
for (const e of recentProdEdges.slice(0, 5)) {
  const src = prodEntities.find(ent => ent.id === e.source_id);
  const tgt = prodEntities.find(ent => ent.id === e.target_id);
  console.log('  ' + (src ? src.name : e.source_id) + ' → ' + (tgt ? tgt.name : e.target_id) + ' (' + e.edge_type + ') - ' + e.created_at);
}
