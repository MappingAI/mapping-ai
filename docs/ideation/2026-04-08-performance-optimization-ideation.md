...
> **Post-implementation refresh (2026-04-19):** Idea 1 "Deploy-Time Thumbnail Caching" has fully landed. The coverage figures in this doc (49%, 783/1,601) are historical; current state is 51.5% of approved people+orgs cached (80% of orgs, 24% of people — a Wikipedia ceiling). The canonical path is `https://mapping-ai.org/thumbnails/<type>-<id>.{png,jpg}`. The CloudFront subdomain `d1vsiezx2npkka.cloudfront.net` referenced in earlier script runs is retired (DNS NXDOMAIN). `entity.thumbnail_url = ''` is now the "tried, no image" sentinel. The frontend no longer calls `en.wikipedia.org/api`, `upload.wikimedia.org`, or `google.com/s2/favicons` at render time. Full details: [`docs/solutions/integration-issues/thumbnail-pipeline-dead-cloudfront-and-external-fallbacks-2026-04-19.md`](../solutions/integration-issues/thumbnail-pipeline-dead-cloudfront-and-external-fallbacks-2026-04-19.md).
