...
1. **SELECT_1 fields**: Value must be exactly one from the allowed list, or null
2. **SELECT_MULTIPLE fields**: Each comma-separated value must be in the allowed list
3. **SELECT_UP_TO_3 fields**: Maximum 3 values, each must be in the allowed list
4. **FREEFORM fields**: No validation on content, only format (URL validation for FREEFORM_URL)

## Belief Attribution

1. **"Explicitly stated" requires quotes**: If `evidence_source = "Explicitly stated"` but `stance_detail` has no actual quotes, should be "Inferred"
2. **Org beliefs ≠ employee beliefs**: Just because employees believe X doesn't mean the org officially holds that position
3. **Non-advocacy orgs rarely have stances**: Evaluation orgs (METR, Epoch), standards bodies, and government agencies typically don't have belief positions

## Edge Directionality

1. **Source is the actor**: For `employer`, source is the person, target is the org
2. **Funder flows from source**: For `funder`, source is the funder, target is the recipient
3. **Reversed edges break display**: Getting direction backwards breaks the frontend labels

## Data Quality

1. **Threat models max 3**: Reduce to top 3 most prominent concerns
2. **Normalize funding_model**: Use categorical values, not freeform descriptions
3. **Check for staleness**: People change jobs, orgs change positions
