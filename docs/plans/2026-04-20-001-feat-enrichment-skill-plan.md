...
- **`notes_sources` flat shape with optional `field_name` per entry.** Shape: `[{url, snippet, retrieved_at, retriever, field_name?}]`. Forward-compatible with the deferred per-field-provenance keystone: if every source entry carries `field_name`, the future migration is a grouping query, not a data rewrite (resolves origin deferred Q#2).
...
- **Classification:** feeds the snippet bundle to `lib/classify.js` which uses Haiku with the `belief-rubric.md` prompt to produce structured belief enums + confidence + reasoning.
