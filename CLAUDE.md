...
├── src/
│   └── tiptap-notes.js     # TipTap source — rich text editor with @mentions, link popovers
├── api/
...
│   ├── search.js           # Lambda: GET /search — full-text search (tsvector + ILIKE, status filter)
│   ├── admin.js            # Lambda: GET/POST /admin — stats, pending, approve/reject, update, auto map refresh
│   ├── upload.js           # Lambda: POST /upload — thumbnail image upload to S3
│   └── export-map.js       # Shared module: generates map-data.json from DB (used by admin + CI)
├── scripts/
...
|----------|--------|---------|
| `/submit` | POST | Insert form submission (→ submissions table + entity table + LLM review) |
...
| `/admin` | POST | Approve, reject, update, delete entities (auto-refreshes map on approve) |
