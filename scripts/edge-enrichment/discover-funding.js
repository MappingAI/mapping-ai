...
async function processEntity(entity, rds, neon, entityCache, dryRun) {
  ...
  const AI_KEYWORDS =
    /\b(ai|artificial intelligence|machine learning|ml|deep learning|neural|llm|gpt|transformer|nlp|computer vision|robotics|autonomous|semiconductor|chip|gpu|safety|alignment|governance|anthropic|openai|deepmind|meta ai|google ai|microsoft ai)\b/i
  ...
  for (const rel of extraction.relationships) {
    ...
    const isInvalidFunder =
      INVALID_NAMES.includes(funderLower) || INVALID_PATTERNS.some((p) => p.test(rel.funder_name || ''))
    const isInvalidRecipient =
      INVALID_NAMES.includes(recipientLower) || INVALID_PATTERNS.some((p) => p.test(rel.recipient_name || ''))
