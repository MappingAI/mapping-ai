...
for (const edge of neitherEdges.rows) {
  ...
  const sourceMatch =
    exactMap.get(sourceLower) ||
    wordMap.get(sourceLower.split(/\s+/)[0]) ||
    [...exactMap.values()].find(
      (e) => e.name.toLowerCase().includes(sourceLower) || sourceLower.includes(e.name.toLowerCase()),
    )

  const targetMatch =
    exactMap.get(targetLower) ||
    wordMap.get(targetLower.split(/\s+/)[0]) ||
    [...exactMap.values()].find(
      (e) => e.name.toLowerCase().includes(targetLower) || targetLower.includes(e.name.toLowerCase()),
    )
