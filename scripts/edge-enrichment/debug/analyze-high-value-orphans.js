...
for (const edge of highValue.rows) {
  ...
  const sourceInRDS = [...rdsNames.keys()].some(
    (n) => n.includes(edge.source_entity_name.toLowerCase()) || edge.source_entity_name.toLowerCase().includes(n),
  )
  const targetInRDS = [...rdsNames.keys()].some(
    (n) => n.includes(edge.target_entity_name.toLowerCase()) || edge.target_entity_name.toLowerCase().includes(n),
  )
