...
const costs = {
  ...
  get exa_cost() {
    return this.exa_searches * 0.008
  },
  get claude_cost() {
    return (this.claude_input_tokens / 1_000_000) * 3 + (this.claude_output_tokens / 1_000_000) * 15
  },
  get total() {
    return this.exa_cost + this.claude_cost
  },
  summary() {
    return `Exa: ${this.exa_searches} ($${this.exa_cost.toFixed(3)}) | Claude: ${this.claude_calls} ($${this.claude_cost.toFixed(3)}) | Total: $${this.total.toFixed(3)}`
  },
}
...
function _getRoleGuidance(entityType, category) {
  ...
  if (entityType === 'organization') {
    if (cat.includes('frontier') || cat.includes('lab')) {
      return {
        ...
        stanceExample:
          "Advocates for 'responsible scaling' with voluntary safety commitments. Supports targeted disclosure requirements but opposes broad licensing. Has published responsible scaling policy.",
        threatExample:
          'Company research focuses on alignment and interpretability. Has warned about concentration of AI power and autonomous weapons risks.',
        lookFor:
          'safety policies, voluntary commitments, government testimony, published research priorities, hiring for safety roles',
        beliefHint:
          'Labs typically have documented positions from RSPs, blog posts, testimony. Look for explicit safety commitments.',
      }
    } else if (cat.includes('safety') || cat.includes('alignment')) {
      return {
        type: 'AI Safety Organization',
        stanceExample:
          'Advocates for mandatory third-party audits of frontier models. Research supports compute thresholds for regulation.',
        ...
        beliefHint:
          'Safety orgs usually have explicit positions. Check their mission statement, research focus, and any policy recommendations.',
      }
    } else if (cat.includes('think tank') || cat.includes('policy')) {
      return {
        type: 'Think Tank/Policy Org',
        stanceExample:
          'Published policy framework calling for sector-specific AI regulation. Testified before Congress recommending licensing for high-risk applications.',
        threatExample:
          'Reports focus on algorithmic bias, labor displacement, and democratic erosion from AI-generated content.',
        lookFor: 'policy papers, congressional testimony, regulatory comments, public statements, expert convenings',
        beliefHint:
          'Policy orgs publish positions. Look for reports, testimony, op-eds. Their funding model may indicate stance.',
      }
    } else if (cat.includes('vc') || cat.includes('capital') || cat.includes('philanthropy')) {
      return {
        type: 'Funder/Investor',
        stanceExample:
          'Portfolio weighted toward AI safety startups. Investment thesis emphasizes responsible development. Signed voluntary AI safety commitments.',
        threatExample:
          'Funds research on economic disruption and power concentration. Investment criteria includes safety review requirements.',
        lookFor: 'portfolio companies, investment thesis, public statements, signed commitments, fund focus areas',
        beliefHint:
          'Investors reveal stance through portfolio choices (safety vs acceleration), public statements, and signed commitments.',
      }
    } else if (cat.includes('government') || cat.includes('agency')) {
      return {
        type: 'Government/Agency',
        stanceExample:
          'Agency issued binding guidance requiring impact assessments for AI in hiring. Enforcement actions against algorithmic discrimination.',
        threatExample:
          'Regulatory focus on consumer protection, civil rights, and national security implications of AI.',
        lookFor:
          'regulations issued, enforcement actions, public guidance, congressional testimony, interagency participation',
        beliefHint:
          'Government stance shown through regulations, guidance, enforcement. Check official statements and actions.',
      }
    }
    // Default for orgs
    return {
      type: 'Organization',
      stanceExample:
        'Organization has taken public position on AI governance through statements, commitments, or actions.',
      ...
      beliefHint:
        'Look for any public statements, signed letters, partnerships that indicate position on AI governance.',
    }
  }
  ...
  if (cat.includes('policymaker')) {
    return {
      type: 'Policymaker',
      stanceExample:
        'Sponsored AI safety legislation requiring frontier model evaluations. Testified calling for mandatory disclosure. Distinguished from broad restriction by emphasizing narrow scope.',
      threatExample:
        'Bill addresses bioweapon assistance, automated criminal activity, and critical infrastructure risks.',
      lookFor: 'bills sponsored, committee assignments, floor statements, testimony, votes, campaign positions',
      beliefHint:
        'Policymakers have voting records, sponsored bills, public statements. Most have discernible positions even if nuanced.',
    }
  } else if (cat.includes('executive')) {
    return {
      type: 'Executive',
      stanceExample:
        "Advocates for 'responsible scaling' rather than pausing. Company voluntarily committed to safety testing. Testified supporting targeted disclosure but opposing licensing.",
      ...
      beliefHint:
        'Executives often have on-record positions from interviews, testimony, company policies. Check for RSPs, safety commitments.',
    }
  } else if (cat.includes('researcher') || cat.includes('academic')) {
    return {
      type: 'Researcher/Academic',
      stanceExample:
        'Co-authored paper arguing for mandatory third-party audits. Testified recommending compute thresholds. Called voluntary commitments insufficient.',
      threatExample:
        'Research focuses on alignment, interpretability, deceptive AI systems, and recursive self-improvement.',
      lookFor: 'papers, research focus, expert testimony, policy recommendations, institutional affiliations',
      beliefHint:
        'Researchers reveal views through papers, testimony, public commentary. Check research focus and recommendations.',
    }
  } else if (cat.includes('investor')) {
    return {
      type: 'Investor',
      stanceExample:
        'Portfolio weighted toward safety startups. Will not invest without safety review boards. Advocates voluntary standards over regulation.',
      threatExample:
        'Warns about economic disruption from automation. Funds displacement research. Concerned about capability concentration.',
      lookFor: 'portfolio companies, investment thesis, public statements, board seats, fund focus',
      beliefHint:
        'Investors show stance through investments (safety-focused vs acceleration), statements, and signed commitments.',
    }
  } else if (cat.includes('journalist')) {
    return {
      type: 'Journalist',
      stanceExample:
        'Coverage emphasizes AI risks and corporate accountability. Written critically about voluntary safety commitments.',
      threatExample:
        'Reporting focuses on labor displacement, algorithmic bias, corporate lobbying, AI misinformation.',
      lookFor: 'beat/coverage area, notable stories, publications, books authored, expertise areas',
      beliefHint:
        'Journalists reveal stance through coverage patterns, framing, sources quoted. Check overall body of work.',
    }
  } else if (cat.includes('organizer')) {
    return {
      type: 'Organizer/Advocate',
      stanceExample:
        'Leading campaign for algorithmic impact assessments. Coalition opposed industry self-regulation. Advocates worker voice in deployment.',
      threatExample:
        'Focuses on labor rights, algorithmic discrimination, corporate accountability. Campaigned against specific AI tools.',
      lookFor: 'campaigns organized, coalitions built, policy wins, testimony, reports published',
      beliefHint:
        'Advocates have clear positions from campaigns, coalition letters, public statements. Usually explicit.',
    }
  } else if (cat.includes('cultural')) {
    return {
      type: 'Cultural Figure',
      stanceExample:
        'Argued AI development should slow until governance catches up. Signed open letter calling for training pause.',
      threatExample:
        'Writing emphasizes existential risk, loss of human agency, consciousness questions, race to bottom on safety.',
      lookFor: 'books, major essays, public lectures, open letters signed, media appearances',
      beliefHint:
        'Public intellectuals often have explicit positions from books, essays, signed letters, public debates.',
    }
  }
...
async function getEdgeContext(entityName) {
  try {
    ...
    const edges = result.rows.map((e) => {
      ...
      const relationship = isSource ? `${e.edge_type} of ${otherEntity}` : `${e.edge_type} by ${otherEntity}`
...
async function updateEntity(client, entityId, cleanData, sourceUrls) {
  ...
  if (cleanData.twitter) {
    updates.push(`twitter = $${idx++}`)
    values.push(cleanData.twitter.replace(/^@/, ''))
  }
  if (cleanData.bluesky) {
    updates.push(`bluesky = $${idx++}`)
    values.push(cleanData.bluesky.replace(/^@/, ''))
  }

  // Numeric fields
  if (cleanData.importance) {
    updates.push(`importance = $${idx++}`)
    values.push(cleanData.importance)
  }
  if (cleanData.notes_confidence) {
    updates.push(`notes_confidence = $${idx++}`)
    values.push(cleanData.notes_confidence)
  }
  ...
  try {
    for (const entity of toProcess) {
      ...
      if (edgeContext && edgeContext.edges.length > 0) {
        ...
        const otherEntity =
          firstEdge.source_entity_name.toLowerCase() === entity.name.toLowerCase()
      ...
      console.log(`    ── BELIEFS ──`)
      console.log(
        `    Stance: ${cleanData.belief_regulatory_stance || '(none)'} | Risk: ${cleanData.belief_ai_risk || '(none)'} | Timeline: ${cleanData.belief_agi_timeline || '(none)'}`,
      )
      if (cleanData.belief_regulatory_stance_detail)
        console.log(`    Stance detail: ${cleanData.belief_regulatory_stance_detail.substring(0, 100)}...`)
      ...
      if (cleanData.ambiguous_name)
        ambiguous.push({
          id: entity.id,
          original: entity.name,
          corrected: cleanData.corrected_name,
          note: cleanData.ambiguity_note,
        })
      ...
      exportResults.push({
        ...
        ...cleanData,
        notes_sources: sourceUrls.join('; '),
      })

      if (!dryRun) {
        try {
          await updateEntity(client, entity.id, cleanData, sourceUrls)
          enriched++
        } catch (err) {
