...
const ALIAS_MAPPINGS = [
  ...
  {
    alias: 'International Association for Safe and Ethical AI (IASEAI)',
    mapTo: 'International Association for Safe and Ethical AI',
  },
      ...
      if (sourceEdges.rows.length > 0) {
        ...
        for (const edge of sourceEdges.rows) {
          ...
          if (wouldDuplicate.rows.length > 0) {
            ...
            const updateFields = rdsId ? 'source_entity_id = $2, source_entity_name = $3' : 'source_entity_name = $3'
            await neon.query(
              `
              UPDATE edge_discovery
      ...
      if (targetEdges.rows.length > 0) {
        ...
        for (const edge of targetEdges.rows) {
          ...
          if (wouldDuplicate.rows.length > 0) {
            ...
            const updateFields = rdsId ? 'target_entity_id = $2, target_entity_name = $3' : 'target_entity_name = $3'
            await neon.query(
              `
              UPDATE edge_discovery
              SET ${updateFields}, updated_at = NOW()
