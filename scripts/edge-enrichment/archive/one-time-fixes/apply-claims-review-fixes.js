...
const CONFIDENCE_DOWNGRADES = [
  ...
  {
    claim_id: '2227_agi_definition_src-bb8e9897f094',
    newConfidence: 'low',
    reason: 'Third-party summary (atlascomputing.org), not ARIA',
  },
  ...
  {
    claim_id: '2300_agi_timeline_src-9c1846dcd6b1',
    newConfidence: 'low',
    reason: 'Third-party blog (forwardfuture.ai)',
  },
]
...
async function main() {
  ...
  for (const claimId of CLAIMS_TO_DELETE) {
    const claim = await neon.query('SELECT claim_id, entity_name, belief_dimension FROM claim WHERE claim_id = $1', [
      claimId,
    ])
  ...
  for (const downgrade of CONFIDENCE_DOWNGRADES) {
    ...
    if (!dryRun) {
      await neon.query('UPDATE claim SET confidence = $1 WHERE claim_id = $2', [
        downgrade.newConfidence,
        downgrade.claim_id,
      ])
