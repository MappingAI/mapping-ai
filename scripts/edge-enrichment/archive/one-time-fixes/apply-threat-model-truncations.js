...
async function main() {
  ...
  for (const t of TRUNCATIONS) {
    const entity = await rds.query('SELECT id, name, belief_threat_models FROM entity WHERE id = $1', [t.id])
    ...
    if (!dryRun) {
      await rds.query('UPDATE entity SET belief_threat_models = $1 WHERE id = $2', [newValue, t.id])
      updated++
    }
  }
