...
const FIELD_FIXES = [
  ...
  // Stance fixes
  {
    ...
    value: 'Light-touch',
    note: 'Saudi Infra fund - Light-touch not Accelerate',
  },
  {
    id: 2190,
    field: 'belief_regulatory_stance',
    value: 'Light-touch',
    note: 'Thiel Foundation - Light-touch not Accelerate',
  },
  {
    id: 2318,
    field: 'belief_regulatory_stance',
    value: 'Mixed/unclear',
    note: 'Waymo - compliance-driven not ideological',
  },
...
async function main() {
  ...
  for (const del of DELETIONS) {
    ...
    // Check for edges
    const edgeCount = await rds.query('SELECT COUNT(*) as count FROM edge WHERE source_id = $1 OR target_id = $1', [
      del.id,
    ])
