...
async function main() {
  ...
  const fundingEdges = fundingResult.rows.map((e) => {
    ...
    const recipientEntity = e.recipient_id
      ? entityById.get(e.recipient_id)
      : entityByName.get(e.recipient.toLowerCase())
  ...
  const funders = Array.from(funderStats.values())
    .map((s) => ({
      ...
      mean_recipient_stance:
        s.recipient_stances.length > 0
  ...
  const funderByYear = Array.from(funderYearStats.values()).sort((a, b) => a.year - b.year || b.count - a.count)
