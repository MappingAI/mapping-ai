...
async function reviewPeople() {
  ...
  try {
    ...
    for (let i = 0; i < people.rows.length; i += 5) {
      ...
      try {
        const response =
          await askClaude(`You are a fact-checker reviewing a database of people in the US AI policy landscape. For each person below, verify the data and return corrections as a JSON array. ONLY include entries that need fixing.
...
async function reviewOrgs() {
  ...
  try {
    ...
    for (let i = 0; i < orgs.rows.length; i += 5) {
      ...
      try {
        const response =
          await askClaude(`You are fact-checking a database of organizations in the US AI policy landscape. Return corrections as a JSON array.
...
async function reviewResources() {
  ...
  try {
    ...
    for (let i = 0; i < resources.rows.length; i += 5) {
      ...
      try {
        const response =
          await askClaude(`You are fact-checking a database of AI policy resources. Return corrections as JSON array.
...
async function main() {
  ...
  execSync(
    'aws s3 cp map-data.json s3://mapping-ai-website-561047280976/map-data.json --content-type application/json',
    { stdio: 'inherit' },
  )
  execSync('aws cloudfront create-invalidation --distribution-id E34ZXLC7CZX7XT --paths "/*"', {
    stdio: 'pipe',
  })
