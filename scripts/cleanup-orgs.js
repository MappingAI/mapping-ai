...
const PARENT_ORGS = [
  ...
  { name: 'Massachusetts Institute of Technology', category: 'Academic', website: 'https://mit.edu' },
...
const PARENT_MAPPINGS = [
  ...
  // MIT
  { pattern: /^MIT\s/i, parent: 'Massachusetts Institute of Technology', exclude: ['Massachusetts Institute of Technology', 'MIT Technology Review'] },
  // Harvard
  ...
  // Cambridge
  { pattern: /Cambridge/i, parent: 'University of Cambridge', exclude: ['University of Cambridge', 'Cambridge Boston Alignment Initiative'] },
  ...
  for (const group of toProcess) {
    ...
    for (const del of toDelete) {
      ...
      if (edgesFrom.rows.length > 0 || edgesTo.rows.length > 0) {
        console.log(`    Edges to redirect: ${edgesFrom.rows.length} outgoing, ${edgesTo.rows.length} incoming`);
      }
      ...
      if (!dryRun) {
        ...
        for (const edge of edgesFrom.rows) {
          try {
            await client.query(
              'UPDATE edge SET source_id = $1 WHERE id = $2',
              [keep.id, edge.id]
            );
            edgesUpdated++;
          } catch (e) {
        ...
        for (const edge of edgesTo.rows) {
          try {
            await client.query(
              'UPDATE edge SET target_id = $1 WHERE id = $2',
              [keep.id, edge.id]
            );
            edgesUpdated++;
          } catch (e) {
        ...
        await client.query(
          'UPDATE entity SET primary_org = $1 WHERE primary_org = $2',
          [keep.name, del.name]
        );
...
async function createSubsidiaryEdges(client) {
  ...
  for (const org of orgsWithParent.rows) {
    ...
    if (existingSet.has(key)) {
      ...
      console.log(`  + ${org.name} → ${org.parent_name} (${dryRun ? 'would create' : 'creating'} edge)`);
      if (!dryRun) {
        await client.query(`
