...
async function migrate() {
  ...
  const entities = JSON.parse(fs.readFileSync('backups/staging-20260415-with-importance/entities.json'));
  ...
  try {
    ...
    await client.query('ALTER TABLE entity ADD CONSTRAINT entity_parent_org_id_fkey FOREIGN KEY (parent_org_id) REFERENCES entity(id)');
    await client.query('ALTER TABLE edge ADD CONSTRAINT edge_source_id_fkey FOREIGN KEY (source_id) REFERENCES entity(id) ON DELETE CASCADE');
    await client.query('ALTER TABLE edge ADD CONSTRAINT edge_target_id_fkey FOREIGN KEY (target_id) REFERENCES entity(id) ON DELETE CASCADE');
    await client.query('ALTER TABLE submission ADD CONSTRAINT submission_entity_id_fkey FOREIGN KEY (entity_id) REFERENCES entity(id)');
    ...
    const pendingCount = await pool.query("SELECT COUNT(*) FROM submission WHERE status = 'pending'");
    ...
    for (const id of samples) {
      ...
      if (dbResult.rows[0] && backupEntity) {
        const match = dbResult.rows[0].name === backupEntity.name && dbResult.rows[0].importance === backupEntity.importance;
        console.log('[' + id + '] ' + dbResult.rows[0].name + ' (importance: ' + dbResult.rows[0].importance + ') ' + (match ? '✓' : '✗'));
      }
    }
