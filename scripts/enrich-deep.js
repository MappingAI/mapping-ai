...
function getRoleGuidance(person) {
  ...
  return {
    ...
    threatExample:
      'Has expressed concern about [specific risks]. Work/statements focus on [specific threat areas].',
    notesExample:
...
function validateResponse(data) {
  ...
  if (data.regulatory_stance_detail && typeof data.regulatory_stance_detail === 'string') {
    clean.regulatory_stance_detail = data.regulatory_stance_detail
      .replace(/^["']|["']$/g, '')
      .substring(0, 2000)
  }
...
async function enrichPerson(client, person) {
  ...
  if (clean.regulatory_stance) {
    ...
    console.log(
      `    stance: ${person.regulatory_stance || '(empty)'} → ${clean.regulatory_stance}${marker}`,
    )
  }
  if (clean.regulatory_stance_detail) {
  ...
  if (clean.evidence_source) {
    ...
    const marker = changed ? ' ← CHANGED' : ''
    console.log(
      `    evidence: ${person.evidence_source || '(empty)'} → ${clean.evidence_source}${marker}`,
    )
  }
  if (clean.agi_timeline) {
  ...
  if (clean.threat_models) {
    console.log(
      `    threats: ${clean.threat_models.substring(0, 150)}${clean.threat_models.length > 150 ? '...' : ''}`,
    )
  }
  if (clean.influence_type) {
