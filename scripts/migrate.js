...
async function migrate() {
  ...
  try {
    ...
    // Provenance fields — added 2026-04-20 for the enrichment skill.
    // Every enrichment carries its Exa/web sources + confidence so the origin
    // of each claim is durable. notes_sources is an array of
    // {url, snippet, retrieved_at, retriever, field_name?} objects.
    await client.query(`ALTER TABLE submission ADD COLUMN IF NOT EXISTS notes_sources JSONB`)
    await client.query(
      `ALTER TABLE submission ADD COLUMN IF NOT EXISTS notes_confidence SMALLINT CHECK (notes_confidence IS NULL OR notes_confidence BETWEEN 1 AND 5)`,
    )
    await client.query(`ALTER TABLE submission ADD COLUMN IF NOT EXISTS enrichment_version TEXT`)
    await client.query(`ALTER TABLE entity     ADD COLUMN IF NOT EXISTS notes_sources JSONB`)
    await client.query(
      `ALTER TABLE entity     ADD COLUMN IF NOT EXISTS notes_confidence SMALLINT CHECK (notes_confidence IS NULL OR notes_confidence BETWEEN 1 AND 5)`,
    )
    await client.query(`ALTER TABLE entity     ADD COLUMN IF NOT EXISTS enrichment_version TEXT`)
      ...
      BEGIN
        IF NEW.entity_id IS NULL AND NEW.status = 'approved' AND OLD.status <> 'approved' THEN
          INSERT INTO entity (
            ...
            notes_sources, notes_confidence, enrichment_version,
            status, qa_approved
          ) VALUES (
            NEW.entity_type,
            ...
            NEW.notes_sources, NEW.notes_confidence, NEW.enrichment_version,
            'approved', true
