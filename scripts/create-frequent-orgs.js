  ...
  try {
    ...
    for (const org of FREQUENT_ORGS) {
      ...
      if (!org.parentName) {
        ...
        const similarExists = existingNames.some(
          (n) => n.includes(baseName) || baseName.includes(n.replace(/\s*\([^)]+\)\s*$/, '')),
        )
    ...
    console.log('===================================================')
    console.log(
      `Orgs ${dryRun ? 'to create' : 'created'}: ${dryRun ? FREQUENT_ORGS.filter((o) => !existingByName.has(o.name.toLowerCase())).length : created}`,
    )
