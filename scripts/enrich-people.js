...
function getRoleGuidance(person) {
  ...
  if (
    ...
    return {
      type: 'Policymaker',
      stanceExample:
        'Sponsored the RAISE Act, which places safety requirements specifically on frontier AI developers who have spent $100M+ on training.',
      threatExample:
        'The RAISE Act was designed to address severe risks including assisting in the creation of bioweapons and automated criminal activity.',
      notesExample:
        'Named to Time 100 AI list (2025); won Future Caucus Rising Star award (2024). Has MS in Computer Science (Georgia Tech).',
      lookFor:
        'bills sponsored/co-sponsored, committee assignments, floor statements, testimony, votes, campaign positions',
    }
  } else if (
    cat.includes('executive') ||
    title.match(/ceo|cto|coo|founder|co-founder|chief|president(?! of)/)
  ) {
    return {
      type: 'Executive',
      stanceExample:
        "Advocates for 'responsible scaling' rather than pausing development. Company voluntarily committed to pre-deployment safety testing.",
      threatExample:
        'Has publicly warned about concentration of AI power in few hands, potential for autonomous weapons.',
      notesExample:
        'Co-founded Anthropic (2021) after leaving OpenAI over safety disagreements. PhD Physics, Princeton.',
      lookFor:
        'company safety policies, voluntary commitments, congressional testimony, blog posts/essays, interviews',
    }
  } else if (
    ...
    return {
      ...
      stanceExample:
        'Co-authored influential paper arguing for mandatory third-party audits of frontier models.',
      threatExample:
        'Research focuses on AI alignment and interpretability. Has published on deceptive AI systems.',
      notesExample:
        'Professor of Computer Science at Stanford; Director of HAI. Over 50,000 citations.',
      lookFor:
        'papers published, research focus, expert testimony, policy recommendations, institutional affiliations',
    }
  } else if (cat.includes('investor')) {
    return {
      type: 'Investor',
      stanceExample:
        'Portfolio heavily weighted toward AI safety startups. Advocates for voluntary industry standards over government regulation.',
      threatExample:
        'Warns about economic disruption from rapid automation. Funds research on AI job displacement.',
      notesExample:
        'General Partner at Founders Fund. Led Series A investments in Anthropic, Cohere.',
      lookFor: 'portfolio companies, investment thesis, public statements, board seats',
    }
  } else if (
    cat.includes('journalist') ||
    title.match(/journalist|reporter|editor|columnist|correspondent/)
  ) {
    return {
      type: 'Journalist',
      stanceExample: 'Coverage tends to emphasize AI risks and corporate accountability.',
      threatExample:
        'Reporting focuses on labor displacement, algorithmic bias, and corporate lobbying.',
      notesExample:
        'AI reporter at New York Times since 2022. Won Pulitzer Prize for series on facial recognition (2023).',
      lookFor: 'beat/coverage area, notable stories broken, publications, awards, books authored',
    }
  } else if (
    cat.includes('organizer') ||
    title.match(/organizer|activist|advocate|director.*union|campaign/)
  ) {
    return {
      type: 'Organizer/Advocate',
      stanceExample: 'Leading campaign for mandatory algorithmic impact assessments.',
      threatExample:
        'Organization focuses on labor rights in the AI era, algorithmic discrimination.',
      notesExample:
        'Executive Director of AI Now Institute since 2020. Former FTC advisor (2018-2020).',
      lookFor: 'organizations led, campaigns organized, coalitions built, policy wins',
    }
  } else if (
    cat.includes('cultural') ||
    title.match(/author|writer|philosopher|intellectual|commentator/)
  ) {
    return {
...
function validateResponse(data) {
  ...
  if (
    data.belief_regulatory_stance_detail &&
    typeof data.belief_regulatory_stance_detail === 'string'
  ) {
...
function getSearchQueries(person) {
  ...
  if (
    ...
    (person.title || '')
      .toLowerCase()
      .match(/senator|congress|assembl|legislat|representative|secretary|commissioner/)
  ) {
...
async function enrichPerson(client, person) {
  ...
  if (
    clean.other_orgs &&
    (!person.other_orgs || clean.other_orgs.length > person.other_orgs.length)
  ) {
