...
const RDS_URL =
  'postgresql://mappingai:WBDMQAmcAiQdOnLSyIdIiZQhOUr1dcX@mapping-ai-db.c9sccou2k3xe.eu-west-2.rds.amazonaws.com:5432/mappingai'
...
function mergeData(neonData, rdsData) {
  ...
  console.log(
    `  People: ${merged.people.length} (Neon: ${neonData.people.length}, RDS: ${rdsData.people.length})`,
  )
  ...
  console.log(
    `  Organizations: ${merged.organizations.length} (Neon: ${neonData.organizations.length}, RDS: ${rdsData.organizations.length})`,
  )
  ...
  console.log(
    `  Resources: ${merged.resources.length} (Neon: ${neonData.resources.length}, RDS: ${rdsData.resources.length})`,
  )
  ...
  console.log(
    `  Relationships: ${merged.relationships.length} (Neon: ${neonData.relationships.length}, RDS: ${rdsData.relationships.length})`,
  )
  ...
  console.log(
    `  Person-organizations: ${merged.person_organizations.length} (Neon: ${neonData.person_organizations.length}, RDS: ${rdsData.person_organizations.length})`,
  )
  ...
  for (const o of merged.organizations) {
    ...
    `,
      [
        ...
        o.thumbnail_url,
        o.regulatory_stance,
        o.regulatory_stance_detail,
        o.evidence_source,
        o.agi_timeline,
        o.ai_risk_level,
        o.threat_models,
        o.submission_count || 0,
        o.status || 'approved',
  ...
  for (const s of merged.submissions) {
    ...
    `,
      [
        ...
        newEntityId,
        s.submitter_email,
        s.submitter_relationship || (s.is_self_submission ? 'self' : 'external'),
        data.name,
        data.title,
        data.category,
        data.primaryOrg,
        ...
        s.notes_html,
        s.notes_mentions,
        data.regulatoryStance,
        STANCE_SCORES[data.regulatoryStance] || null,
        data.agiTimeline,
        TIMELINE_SCORES[data.agiTimeline] || null,
        data.aiRiskLevel,
        RISK_SCORES[data.aiRiskLevel] || null,
        s.status,
        s.llm_review,
...
async function main() {
  ...
  try {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({ neon: neonData, rds: rdsData, merged }, null, 2),
    )
    ...
    console.log('\n=== Next Steps ===')
    console.log(
      '1. Update .env to use RDS: DATABASE_URL="postgresql://mappingai:...@mapping-ai-db...amazonaws.com:5432/mappingai"',
    )
