...
async function main() {
  ...
  console.log(
    `  0 edges:  ${distribution[0]} people (${Math.round((distribution[0] / total) * 100)}%)`,
  )
  console.log(
    `  1 edge:   ${distribution[1]} people (${Math.round((distribution[1] / total) * 100)}%)`,
  )
  console.log(
    `  2 edges:  ${distribution[2]} people (${Math.round((distribution[2] / total) * 100)}%)`,
  )
  console.log(
    `  3 edges:  ${distribution[3]} people (${Math.round((distribution[3] / total) * 100)}%)`,
  )
  console.log(
    `  4+ edges: ${distribution['4+']} people (${Math.round((distribution['4+'] / total) * 100)}%)`,
  )

  console.log('\nORG EDGES PER PERSON (excluding person-to-person edges):')
  console.log(
    `  0 orgs:  ${orgDistribution[0]} people (${Math.round((orgDistribution[0] / total) * 100)}%)`,
  )
  console.log(
    `  1 org:   ${orgDistribution[1]} people (${Math.round((orgDistribution[1] / total) * 100)}%)`,
  )
  console.log(
    `  2 orgs:  ${orgDistribution[2]} people (${Math.round((orgDistribution[2] / total) * 100)}%)`,
  )
  console.log(
    `  3 orgs:  ${orgDistribution[3]} people (${Math.round((orgDistribution[3] / total) * 100)}%)`,
  )
  console.log(
    `  4+ orgs: ${orgDistribution['4+']} people (${Math.round((orgDistribution['4+'] / total) * 100)}%)`,
  )
