...
const EDGE_CONFIDENCE_THRESHOLD = edgeThresholdArg
  ? parseInt(edgeThresholdArg.split('=')[1], 10)
  : 3
...
async function enrichEntity(client, entity, allEntities) {
  ...
  if (lowConfEdges.length > 0) {
    console.log(
      `    Edges skipped (confidence < ${EDGE_CONFIDENCE_THRESHOLD}): ${lowConfEdges.length}`,
    )
