...
import { findEntityMatch, getCanonicalOrgName, getParentOrgName, normalizeOrgName } from './lib/org-matching.js'
...
async function enrichEntity(client, entity, allEntities) {
  ...
  const webContent = uniqueResults
    .slice(0, 12)
    .map((r, i) => {
  ...
  await client.query(
    ...
    WHERE id = $4
  `,
    [data.notes.substring(0, 3000), confidence, JSON.stringify(sourceUrls), entity.id],
  )
  ...
  // Process edges
  const validEdges = (data.extracted_edges || []).filter(
    (e) =>
      e.target_name &&
      e.edge_type &&
  ...
  const lowConfEdges = (data.extracted_edges || []).filter(
    (e) => e.confidence && e.confidence < EDGE_CONFIDENCE_THRESHOLD,
  )
  ...
  for (const edge of result.edges) {
    ...
    if (matchResult) {
      ...
      targetEntity = await evaluateAndCreateEntity(
        ...
        edge.target_type,
        edge.source_quote,
        result.sources[edge.source_number - 1] || '',
        allEntities,
      )
...
async function main() {
  ...
  try {
    const allEntitiesRes = await client.query('SELECT id, name, entity_type FROM entity WHERE status = $1', [
      'approved',
    ])
