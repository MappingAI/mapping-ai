...
function classifyStance(text) {
  ...
  if (
    t.includes('pause') ||
    t.includes('moratorium') ||
    t.includes('precaution') ||
    t.includes('halt')
  )
  ...
  if (
    ...
    return 'Targeted'
  if (
    t.includes('light-touch') ||
    t.includes('voluntary') ||
    t.includes('self-govern') ||
    t.includes('industry-led')
  )
    return 'Light-touch'
  if (
    ...
    return 'Accelerate'
  if (t.includes('nationali') || t.includes('public control') || t.includes('public utility'))
    return 'Nationalize'
  if (
    t.includes('nuanced') ||
    t.includes('mixed') ||
    t.includes('complicated') ||
    t.includes('evolving')
  )
...
function classifyTimeline(text) {
  const t = text.toLowerCase()
  if (t.includes('already here') || t.includes('already achieved') || t.includes('agi is here'))
    return 'Already here'
  if (
    /\b(2026|2027|2028)\b/.test(t) ||
    t.includes('2-3 year') ||
    t.includes('two to three') ||
    t.includes('imminent')
  )
    return '2-3 years'
  if (
    ...
    return '5-10 years'
  if (t.includes('10-25') || t.includes('decades away') || /\b(204\d|205\d)\b/.test(t))
    return '10-25 years'
  if (
    t.includes('25+') ||
    t.includes('never') ||
    t.includes('not in our lifetime') ||
    t.includes('very far')
  )
...
function classifyThreats(text) {
  ...
  if (
    ...
    t.includes('authorit') ||
    t.includes('civil libert')
  )
    found.push('Democratic erosion')
  if (t.includes('cyber') || t.includes('hack') || t.includes('security vulnerability'))
    found.push('Cybersecurity')
  if (
    t.includes('misinfo') ||
    t.includes('deepfake') ||
    t.includes('disinform') ||
...
function classifyInfluence(text, title) {
  ...
  if (t.includes('advis') || t.includes('strateg') || t.includes('consult'))
    found.push('Advisor/strategist')
  if (
    t.includes('research') ||
...
function classifyPersonCategory(text, name, title) {
  ...
  if (
    ...
    return 'Academic'
  if (t.includes('researcher') || t.includes('scientist') || t.includes('phd') || t.includes('lab'))
    return 'Researcher'
  if (
    t.includes('senator') ||
    t.includes('congress') ||
  ...
  if (
    ...
    t.includes('thought leader') ||
    t.includes('public figure')
  )
...
async function loadOrgLookup(client) {
  const r = await client.query(
    "SELECT id, LOWER(name) as name FROM organizations WHERE status = 'approved'",
  )
  orgNameToId = {}
  r.rows.forEach((o) => {
  ...
  try {
    ...
    for (const person of result.rows) {
      ...
      try {
        ...
        const category =
          person.category || classifyPersonCategory(mainText, person.name, person.title)
