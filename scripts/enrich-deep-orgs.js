...
const VALID_FUNDING = ['Venture-backed', 'Revenue-generating', 'Government-funded', 'Philanthropic', 'Membership', 'Mixed', 'Public benefit', 'Self-funded', 'Other'];
...
function getOrgGuidance(org) {
  ...
  if (cat.includes('frontier lab')) {
    return {
      ...
      stanceExample: "Advocates for 'responsible scaling' with voluntary safety commitments and pre-deployment testing. Supports targeted disclosure requirements but opposes broad licensing schemes.",
      threatExample: "Company safety team focuses on alignment research, red-teaming for dangerous capabilities (bio, cyber), and preventing misuse. Published responsible scaling policy with capability thresholds.",
      notesExample: "Founded 2015 as nonprofit, restructured 2019. Launched ChatGPT (Nov 2022), GPT-4 (Mar 2023). 1,500+ employees, valued at $80B+ (2024). HQ in San Francisco. Key products: ChatGPT, API, DALL-E.",
      lookFor: "products/models released, funding raised, valuation, employee count, safety policies, government testimony, voluntary commitments"
    };
  } else if (cat.includes('safety') || cat.includes('alignment')) {
    return {
      type: 'AI Safety/Alignment Organization',
      categoryCorrect: 'AI Safety/Alignment',
      stanceExample: "Advocates for mandatory safety evaluations and third-party audits before deploying frontier models. Publishes technical alignment research and policy recommendations.",
      threatExample: "Research focuses on alignment techniques (RLHF, constitutional AI, interpretability), deceptive alignment risks, and evaluating dangerous capabilities in frontier models.",
      notesExample: "Founded 2017, nonprofit. 25 researchers. Funded by Open Philanthropy, SFF. Key publications: 'Concrete Problems in AI Safety', interpretability research. Partners with Anthropic, DeepMind on safety evals.",
      lookFor: "research focus areas, key publications, funding sources, team size, partnerships with labs, policy recommendations"
    };
  } else if (cat.includes('think tank') || cat.includes('policy')) {
    return {
      type: 'Think Tank/Policy Organization',
      categoryCorrect: 'Think Tank/Policy Org',
      stanceExample: "Publishes policy briefs advocating for risk-based regulatory framework with mandatory incident reporting and pre-deployment assessments for high-risk AI systems.",
      threatExample: "Research covers AI governance gaps, recommends export controls on AI chips, algorithmic accountability frameworks, and workforce transition policies.",
      notesExample: "Founded 2018, nonprofit. Based in Washington DC. Staff of 15. Funded by Ford Foundation, Hewlett. Key reports: 'AI Governance Roadmap' (2024). Testified before Senate Commerce Committee.",
      lookFor: "policy papers published, congressional testimony, funding sources, political lean, key staff, influence on legislation"
    };
  } else if (cat.includes('vc') || cat.includes('capital') || cat.includes('philanthrop') || cat.includes('funder')) {
    return {
      type: 'VC/Funder/Incubator',
      categoryCorrect: 'VC/Capital/Philanthropy',
      stanceExample: "Portfolio includes both AI capabilities and safety companies. Investment thesis emphasizes 'responsible AI' as market opportunity. No explicit policy advocacy.",
      threatExample: "Funds startups working on AI safety tooling, interpretability platforms, and governance software. Treats safety risks as business opportunities rather than existential concerns.",
      notesExample: "Founded 2015, $500M AUM. 40 portfolio companies, 5 unicorns. Key AI investments: Anthropic (Series A), Cohere. Partners: Jane Doe, John Smith. Based in SF.",
      lookFor: "AUM, portfolio companies, AI-specific investments, fund partners, investment thesis, incubator programs, co-investors"
    };
  } else if (cat.includes('government') || cat.includes('agency')) {
    return {
      type: 'Government Agency',
      categoryCorrect: 'Government/Agency',
      stanceExample: "Implements AI executive orders, develops federal AI use guidelines, coordinates agency AI adoption. Balances innovation promotion with risk management.",
      threatExample: "Regulatory focus on algorithmic discrimination, AI in critical infrastructure, federal procurement standards, and national security applications.",
      notesExample: "Established 2021 under OSTP. Coordinates federal AI policy across 20+ agencies. Published AI Bill of Rights (Oct 2022). Director: Jane Smith (appointed 2023).",
      lookFor: "regulatory authority, key rules/guidelines issued, leadership, budget, enforcement actions, international coordination"
    };
  } else if (cat.includes('academic')) {
    return {
      type: 'Academic Institution/Lab',
      categoryCorrect: 'Academic',
      stanceExample: "Faculty publish research on AI governance frameworks. Lab director testified advocating for mandatory pre-deployment audits and compute governance.",
      threatExample: "Research covers technical AI safety, societal impacts, labor displacement modeling, and AI ethics. Trains next generation of AI policy researchers.",
      notesExample: "Founded 2019, housed in CS department. 8 faculty, 30 PhD students. Funded by NSF, Open Philanthropy. Key publications: 150+ papers on AI safety/governance. Director: Prof. Jane Smith.",
      lookFor: "faculty, research areas, key publications, funding sources, PhD students, policy influence, industry partnerships"
    };
  } else if (cat.includes('media') || cat.includes('journalism')) {
    return {
      type: 'Media/Journalism Organization',
      categoryCorrect: 'Media/Journalism',
      stanceExample: "Editorial coverage tends to emphasize AI risks and corporate accountability. Investigative reporting on AI lab safety practices.",
      threatExample: "Coverage focuses on labor impacts, algorithmic bias, misinformation, and corporate influence on AI policy.",
      notesExample: "AI coverage team of 5 reporters. Won Pulitzer for AI accountability series (2024). Key beat reporters: Jane Smith, John Doe. Podcast: 'AI Explained' (100K subscribers).",
      lookFor: "AI beat reporters, notable stories/investigations, editorial stance, audience reach, awards, podcasts/newsletters"
    };
  } else if (cat.includes('labor') || cat.includes('civil') || cat.includes('union')) {
    return {
      type: 'Labor/Civil Society Organization',
      categoryCorrect: 'Labor/Civil Society',
      stanceExample: "Advocates for worker voice in AI deployment decisions, algorithmic transparency in hiring, and transition support for displaced workers.",
      threatExample: "Campaigns focus on AI-driven surveillance, algorithmic management, gig worker classification, and ensuring workers benefit from AI productivity gains.",
      notesExample: "Founded 2020, coalition of 15 unions. 500K members. Key campaigns: 'AI Workers Bill of Rights', Amazon warehouse algorithm transparency. Director: Jane Smith.",
    ...
    return {
      ...
      stanceExample: "Advocates for mandatory algorithmic impact assessments, bias audits, and community consent before AI deployment in public services.",
      threatExample: "Research documents discriminatory AI systems in criminal justice, hiring, healthcare. Campaigns against facial recognition, predictive policing.",
      notesExample: "Founded 2017, nonprofit. Staff of 20. Key reports: 'Gender Shades' study on facial recognition bias. Director: Dr. Jane Smith. Funded by Ford, MacArthur foundations.",
    ...
    return {
      ...
      stanceExample: "Supports candidates favoring light-touch AI regulation and innovation-friendly policies. Opposes state-level AI safety bills.",
      threatExample: "Lobbying focuses on preventing regulatory burden on AI companies, promoting federal preemption of state AI laws.",
      notesExample: "Super PAC founded 2024. Raised $10M. Major donors: tech executives. Spent $5M on 2024 House races. Opposes SB 1047-style legislation.",
      lookFor: "donations received, spending, candidates supported/opposed, lobbying targets, major donors, policy positions"
    };
  }
