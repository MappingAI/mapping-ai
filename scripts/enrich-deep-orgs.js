...
function getOrgGuidance(org) {
  ...
  if (cat.includes('frontier lab')) {
    ...
    return {
      ...
      lookFor:
        'member unions/orgs, campaigns, policy wins, leadership, funding, coalition partners',
    }
  } else if (cat.includes('ethics') || cat.includes('bias') || cat.includes('rights')) {
    return {
      ...
      lookFor:
        'research/audits published, campaigns, litigation, policy influence, leadership, funding sources',
    }
  } else if (cat.includes('political') || cat.includes('campaign') || cat.includes('pac')) {
    return {
  ...
  return {
    ...
    notesExample:
      'Founded [year], [type]. [Size]. [Funding]. Key activities: [main work]. [Location].',
...
async function enrichOrg(client, org) {
  ...
  if (clean.regulatory_stance) {
    ...
    const marker = changed ? ' ← CHANGED' : ''
    console.log(
      `    stance: ${org.regulatory_stance || '(empty)'} → ${clean.regulatory_stance}${marker}`,
    )
  }
  if (clean.regulatory_stance_detail) {
