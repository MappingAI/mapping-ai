...
async function main() {
  ...
  if (badEdges.rows.length > 0) {
    ...
    for (const e of badEdges.rows.slice(0, 10)) {
      console.log(`   Edge ${e.id}: ${e.source_name} (${e.source_status}) → ${e.target_name} (${e.target_status})`);
    }
  ...
  for (const edge of mapData.edges) {
    if (!exportedIds.has(edge.source_id) || !exportedIds.has(edge.target_id)) {
      ...
      if (missingInExport <= 10) {
        console.log(`   Edge ${edge.id}: source=${edge.source_id} (${exportedIds.has(edge.source_id) ? 'ok' : 'MISSING'}), target=${edge.target_id} (${exportedIds.has(edge.target_id) ? 'ok' : 'MISSING'})`);
      }
    }
  }
  ...
  for (const partyName of parties) {
    const party = await client.query(
      `SELECT id, name, status FROM entity WHERE name = $1`,
      [partyName]
    );
  ...
  for (const p of policymakers.rows.slice(0, 10)) {
    console.log(`   [${p.id}] ${p.name} (${p.edge_count} total edges, primary_org: ${p.primary_org || 'null'})`);
  }
