...
async function analyze() {
  ...
  for (const org of orgs.rows) {
    const words = org.name
      .toLowerCase()
      .split(/\s+/)
      .filter((w) => !['the', 'a', 'an', 'of', 'for', 'and', 'in', 'on', 'at'].includes(w))
  ...
  for (const [key, group] of byFirstWord) {
    if (group.length > 1) {
      ...
      for (const org of group) {
        ...
        console.log(
          `       category: ${org.category || '(none)'}, website: ${org.website || '(none)'}`,
        )
      }
      console.log('')
    }
  ...
  for (const org of orgs.rows) {
    ...
    const looksLikePersonName =
      words.length === 2 && words.every((w) => /^[A-Z][a-z]+$/.test(w)) && !hasOrgKeyword
  ...
  for (const person of people.rows) {
    ...
    const containsMatch = [...orgNamesLower].some(
      (on) => on.includes(poLower) || poLower.includes(on),
    )
  ...
  for (const row of edgeStats.rows) {
    console.log(
      `  ${row.edge_type} (${row.created_by}): ${row.total} edges from ${row.unique_sources} sources`,
    )
  }
  ...
  const needsParent = []
  const orgNames = orgs.rows.map((o) => ({
    id: o.id,
    name: o.name,
  ...
  for (const org of orgNames) {
    ...
    for (const potentialParent of orgNames) {
      ...
      if (
        org.nameLower.includes(potentialParent.nameLower) ||
        (org.nameLower.startsWith(firstParentWord) && firstParentWord.length > 5)
      ) {
        // Make sure the potential child looks like a sub-org
        const looksLikeSubOrg =
          subOrgPatterns.some((p) => org.nameLower.includes(p)) ||
