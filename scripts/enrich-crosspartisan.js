...
const SEARCH_QUERIES = {
  state_preemption: {
    person: (name) => `"${name}" AI federal preemption state regulation California SB 1047 position`,
    org: (name) => `"${name}" AI federal preemption state regulation position advocacy`,
  },
  open_source_weights: {
    person: (name) => `"${name}" open source AI open weights Llama frontier model restriction position`,
    org: (name) => `"${name}" open source AI open weights frontier model restriction advocacy`,
  },
  compute_governance: {
    person: (name) => `"${name}" AI compute threshold FLOPs reporting training run executive order`,
    org: (name) => `"${name}" AI compute threshold FLOPs reporting training run policy`,
  },
  export_controls_chips: {
    person: (name) => `"${name}" AI chip export controls China BIS semiconductor position`,
    org: (name) => `"${name}" AI chip export controls China BIS semiconductor policy`,
  },
  pre_deployment_testing: {
    person: (name) => `"${name}" AI safety testing pre-deployment evaluation red team AISI NIST`,
    org: (name) => `"${name}" AI safety testing pre-deployment evaluation advocacy position`,
  },
  liability: {
    person: (name) => `"${name}" AI developer liability product liability Section 230 harm`,
    org: (name) => `"${name}" AI liability developer accountability product liability position`,
  },
}

const ORG_CATEGORIES = ['Think Tank/Policy Org', 'Political Campaign/PAC', 'Labor/Civil Society', 'Ethics/Bias/Rights']

const EXTRACTION_PROMPT = `You are analyzing web search results about an entity's positions on specific AI policy issues.

For each policy area below, determine if the search results contain evidence of this entity's position. Only extract claims where you find DIRECT evidence.
...
async function getTargetEntities(resumeSet) {
  const orgPlaceholders = ORG_CATEGORIES.map((_, i) => `$${i + 1}`).join(', ')
  const r = await prodDb.query(
    `
    SELECT e.id, e.name, e.category, e.entity_type
    FROM entity e
    WHERE e.status = 'approved' AND (
      (e.entity_type = 'person' AND e.category = 'Policymaker')
      OR (e.entity_type = 'organization' AND e.category IN (${orgPlaceholders}))
    )
  ...
  let entities = r.rows.map((row) => ({
    ...
    party: row.entity_type === 'person' ? partyMap.get(row.id) || null : null,
  }))
...
async function searchPolicyAreas(entity) {
  const queryVariant = entity.entity_type === 'organization' ? 'org' : 'person'
  const results = []
  for (const area of POLICY_AREAS) {
    const querySet = SEARCH_QUERIES[area.id]
    if (!querySet) continue
    const queryFn = querySet[queryVariant]
    try {
      ...
      console.error(`    Exa error ${entity.name} / ${area.id}: ${err.message}`)
    }
    await delay(150)
  }
  return results
...
async function extractClaims(entity, searchResults) {
  ...
  const entityLabel = entity.entity_type === 'organization' ? 'Organization' : 'Person'
  const partyLine = entity.party ? `\nParty: ${entity.party}` : ''
  const prompt = `${entityLabel}: ${entity.name}${partyLine}\nCategory: ${entity.category}\nType: ${entity.entity_type}\n\nSEARCH RESULTS:\n${sourcesText}`
...
async function main() {
  ...
  const policymakers = entities.filter((e) => e.entity_type === 'person')
  const orgs = entities.filter((e) => e.entity_type === 'organization')
  console.log(`Targets: ${entities.length} entities (${policymakers.length} policymakers, ${orgs.length} orgs)`)
  ...
  try {
    for (const entity of entities) {
      processed++
      const typeLabel = entity.entity_type === 'organization' ? 'org' : entity.party || '?'
      console.log(`\n[${processed}/${entities.length}] ▸ ${entity.name} (${typeLabel}, id=${entity.id})`)

      const results = await searchPolicyAreas(entity)
