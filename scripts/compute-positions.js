...
export function computePositions(data) {
  ...
  data.people.forEach((d) => {
    ...
    if (primaryOrg) {
      ...
      const orgMatch = data.organizations.find(
  ...
  while (remaining.size > 0) {
    ...
    for (const cat of remaining) {
      ...
      if (
        score > bestScore ||
        (score === bestScore && (catCounts[cat] || 0) > (catCounts[bestCat] || 0))
      ) {
        bestScore = score
        bestCat = cat
      }
  ...
  const entityById = {}
  nodes.forEach((d) => {
    if (d.id) entityById[`${d.entityType}-${d.id}`] = d
  })
  ...
  nodes.forEach((d) => {
    if (d.isResource) {
      ...
      const center = clusterCenters[d.clusterKey] ||
        clusterCenters[d.category] || { x: centerX, y: centerY }
      d.x = center.x + (seededRandom() - 0.5) * 60
      d.y = center.y + (seededRandom() - 0.5) * 60
    }
  })
  ...
  const orgByName = {}
  nodes
    .filter((n) => n.entityType === 'organization')
    .forEach((o) => {
      orgByName[o.name.toLowerCase()] = o
    })
  nodes
    .filter((n) => n.entityType === 'person' && n.primary_org)
    .forEach((p) => {
  ...
  const sim = forceSimulation(nodes)
    .force(
      'charge',
      forceManyBody().strength((d) => (d.entityType === 'organization' ? -4 : -1.5)),
    )
    .force(
      'collision',
      forceCollide()
