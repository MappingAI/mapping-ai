...
function mapOrgCategory(csvCats) {
  ...
  if (
    cats.some((c) =>
      ['Empirical research', 'Conceptual research', 'Capabilities research'].includes(c),
    )
  )
...
async function importData() {
  ...
  try {
    ...
    console.log(
      `Existing: ${existingOrgNames.size} orgs, ${existingResourceNames.size} resources\n`,
    )
    ...
    for (const row of aiSafetyRows) {
      ...
      if (isResource && !isOrg) {
        ...
        const notes = description
          ? `From AISafety.com map. ${description}`
          : 'From AISafety.com map.'
        ...
        // → Organizations table
        if (existingOrgNames.has(name.toLowerCase())) {
          skippedDupes++
          continue
        }
    ...
    for (const row of readingsRows) {
      ...
      if (!title) continue
      if (existingResourceNames.has(title.toLowerCase())) {
