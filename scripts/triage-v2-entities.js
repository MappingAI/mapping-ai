...
async function main() {
  ...
  for (const e of entities.rows) {
    ...
    // AI-related keywords
    const aiKeywords =
      /(artificial intelligence|machine learning|deep learning|neural network|nlp|natural language|computer vision|robotics|autonomous|llm|language model|ai safety|ai alignment|ai policy|ai governance|ai ethics)/
    ...
    const knownAI =
      /(deepmind|openai|anthropic|meta ai|google brain|stanford hai|berkeley ai|mit csail|miri|chai|alignment|safety institute)/i
    const fromKnownAI = knownAI.test(sources)

    // Red flags - probably not AI-relevant
    const redFlags =
      /(attorney general|county|sheriff|real estate|hospital|medical center|\bbank\b|airline|hotel|restaurant|church|\bsports\b)/i
