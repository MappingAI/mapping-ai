...
async function loadExisting(client) {
  ...
  console.log(
    `Loaded existing: ${existingResourceTitles.size} resources, ${existingPeopleNames.size} people, ${existingOrgNames.size} orgs\n`,
  )
}
...
function isDupeResource(title) {
  ...
  if (t.length > 20) {
    for (const existing of existingResourceTitles) {
      if (existing.includes(t.substring(0, 20)) || t.includes(existing.substring(0, 20)))
        return true
    }
  }
  return false
...
function classifyResourceType(url, title, text) {
  ...
  if (
    all.includes('arxiv') ||
    all.includes('paper') ||
    all.includes('journal') ||
    all.includes('doi.org')
  )
  ...
  if (
    ...
    all.includes('nytimes') ||
    all.includes('washingtonpost') ||
    all.includes('reuters')
  )
    return 'News Article'
  return 'Essay'
}
...
function classifyResourceCategory(text) {
  ...
  if (t.includes('labor') || t.includes('job') || t.includes('worker') || t.includes('econom'))
    return 'Labor & Economy'
  if (
    t.includes('security') ||
    t.includes('military') ||
    t.includes('defense') ||
  ...
  if (t.includes('ethic') || t.includes('bias') || t.includes('fairness') || t.includes('rights'))
...
async function discoverResources(limit) {
  ...
  try {
    ...
    const searches = [
      {
        query: 'influential AI policy paper report 2024 2025 regulation governance',
        numResults: 15,
        category: 'research_paper',
      },
      { query: 'AI governance framework white paper recommendation 2024 2025', numResults: 10 },
      {
        query: 'AI safety alignment important research paper 2024 2025',
        numResults: 10,
        category: 'research_paper',
      },
      ...
      {
        query: 'artificial intelligence societal impact labor displacement study report',
        numResults: 10,
      },
