...
async function main() {
  ...
  try {
    ...
    for (const h of HIERARCHY) {
      ...
      // Find children
      const children = orgs.rows.filter(
        (o) =>
          h.childPattern.test(o.name) && !h.exclude.includes(o.name) && o.name !== h.parent.name,
      )
    ...
    console.log('=============================')
    console.log(
      `Parent orgs ${dryRun ? 'to create' : 'created'}: ${dryRun ? HIERARCHY.filter((h) => !h.parent.exists && !orgByName.has(h.parent.name.toLowerCase())).length : parentsCreated}`,
    )
    console.log(
      `Children ${dryRun ? 'to link' : 'linked'}: ${dryRun ? '(see above)' : childrenLinked}`,
    )
    console.log(
      `Edges ${dryRun ? 'to create' : 'created'}: ${dryRun ? '(same as children)' : edgesCreated}`,
    )
