...
async function fillPeopleOrgs() {
  ...
  try {
    ...
    for (const p of people.rows) {
      try {
        const res = await exaSearch(`"${p.name}" works at organization affiliation`, { numResults: 2 });
        ...
        if (orgName) {
          ...
          if (org) {
            try {
              await client.query('INSERT INTO person_organizations (person_id, organization_id, role, is_primary) VALUES ($1, $2, $3, true) ON CONFLICT DO NOTHING', [p.id, org.id, p.title || null]);
...
async function fillLocations() {
  ...
  try {
    ...
    for (const p of people.rows) {
      try {
        const res = await exaSearch(`"${p.name}" based located city`, { type: 'fast', numResults: 2 });
        const text = res.results.flatMap(r => r.highlights || []).join(' ');
        ...
        for (const pat of locPatterns) {
          const m = text.match(pat);
          if (m) { location = m[1] || m[0]; break; }
        }

        if (location) {
          // Normalize common locations
          if (/washington/i.test(location)) location = 'Washington, DC';
          if (/san francisco/i.test(location) || /sf/i.test(location)) location = 'San Francisco, CA';
    ...
    for (const o of orgsNoLoc.rows) {
      try {
        const res = await exaSearch(`"${o.name}" headquarters located city`, { type: 'fast', numResults: 2 });
        const text = res.results.flatMap(r => r.highlights || []).join(' ');
        const locPatterns = [
        ...
        if (location) {
          ...
          await client.query('UPDATE organizations SET location = $1 WHERE id = $2', [location, o.id]);
          updates++;
    ...
    for (const p of people.rows) {
      try {
        ...
        for (const r of res.results) {
          ...
          if (m && !['search','hashtag','i','explore','home','intent','status'].includes(m[1])) {
...
async function fillResourceGaps() {
  ...
  try {
    ...
    for (const r of noUrl.rows) {
      try {
        ...
        if (res.results[0]?.url) {
          await client.query('UPDATE resources SET url = $1 WHERE id = $2', [res.results[0].url, r.id]);
          updates++;
    ...
    for (const r of noAuthor.rows) {
      try {
        const res = await exaSearch(`"${r.title}" author by written`, { type: 'fast', numResults: 2 });
        const text = res.results.flatMap(x => x.highlights || []).join(' ');
        const m = text.match(/\bby\s+([A-Z][a-z]+ [A-Z][a-z]+(?:\s+(?:and|&)\s+[A-Z][a-z]+ [A-Z][a-z]+)?)/);
        if (m) {
...
async function buildLinks() {
  ...
  try {
    ...
    const people = await client.query("SELECT id, name, title, primary_org, other_orgs FROM people WHERE status='approved' AND primary_org IS NOT NULL AND primary_org != ''");
    console.log(`\n── Building person-org links for ${people.rows.length} people ──\n`);

    let linked = 0;
    for (const p of people.rows) {
      for (const orgText of [p.primary_org, ...(p.other_orgs || '').split(/[,;]/)].filter(Boolean)) {
        const t = orgText.toLowerCase().trim();
        const org = orgList.find(o => {
          const ol = o.name.toLowerCase();
          return t.includes(ol) || ol.includes(t) ||
            t.split(/\s+/).filter(w => w.length > 3).some(w => ol.includes(w));
        });
        if (org) {
          try {
            const isPrimary = orgText === p.primary_org;
            await client.query('INSERT INTO person_organizations (person_id, organization_id, role, is_primary) VALUES ($1, $2, $3, $4) ON CONFLICT DO NOTHING',
