...
console.log(`→ If Open Phil withdraws: ${openPhilOrgs.size} orgs affected, ${orgsOnlyOpenPhil.length} with no other funders`)
...
funderStats.slice(0, 15).forEach(f => {
  console.log(`  ${f.funder.slice(0, 43).padEnd(43)} ${String(f.orgs).padStart(3)}   ${String(f.grants).padStart(4)}   ${f.categories}/3`)
})
...
singleCategoryOrgs.sort((a, b) => b.funders - a.funders).slice(0, 8).forEach(o => {
  console.log(`  ${o.org}: all ${o.funders} funders are ${o.category}`)
})

console.log(`\nOrgs with diverse funder types (3+ different categories):`)
multiCategoryOrgs.sort((a, b) => b.categories.length - a.categories.length).slice(0, 8).forEach(o => {
