...
if (!limit && !allMode && !singleId) {
  console.log(
    'Usage: PILOT_DB="..." node scripts/enrich-claims.js [--limit=N | --all | --id=N] [--resume] [--dry-run] [--type=person|organization]',
  )
  process.exit(0)
}
...
async function getTargetEntities(resumeSet) {
  let query = `
    SELECT id, entity_type, name, category, website, resource_url,
  ...
  if (singleId) {
    query = `SELECT id, entity_type, name, category, website, resource_url,
             belief_regulatory_stance, belief_agi_timeline, belief_ai_risk,
             belief_evidence_source, belief_regulatory_stance_detail
...
async function main() {
  ...
  console.log(`Targets: ${entities.length} entities (belief_evidence_source = 'Explicitly stated')`)
